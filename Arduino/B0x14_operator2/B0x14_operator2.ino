...
#define SEEK_IN 12
#define SEEK_OUT 11
#define I2C_SLAVE_ADDRESS 0x14 //1
...
void updateValue () {
  ...
  int input1v = analogRead(A3); //value
  int pot_v = analogRead(A2); //operation
  int input2v = analogRead(A1); //variable
  ...
  inputs[0] = (in1) ? map(input1v, 0, 1023, 0, 9) : 255;
  inputs[1] = (in2) ? convertVar(input2v) : 0;
  inputs[2] = map((pot), 0, 1023, 0, 255);
